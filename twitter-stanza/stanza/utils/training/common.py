...
from stanza.models.ner import model
...
def build_argparse():
    ...
    parser.add_argument('--save_name', type=str, default=None, help="name for saving models.  If set, will override the model's default.")
    ...
    for treebank_idx, treebank in enumerate(treebanks):
        ...
        if mode == Mode.TRAIN and not command_args.force and model_name != 'ete':
            if command_args.save_name:
                ...
                save_name = "%s_%s"%(short_name, model_name)
            if command_args.save_dir:
                model_path = "%s/%s.pt" % (command_args.save_dir, save_name)
            else:
                model_path = "saved_models/%s/%s.pt" % (model_dir, save_name)
